...
class Dumper
  ...
  def to_ynab_transaction(transaction)
    ...
    begin
       if transaction.description == "SONSTIGER EINZUG"
          puts "IGNORE RECORD: " + JSON.pretty_generate(transaction)
          return nil
       end
    rescue
       puts "NO DESCRIPTION FOR: " + JSON.pretty_generate(transaction)
    end
